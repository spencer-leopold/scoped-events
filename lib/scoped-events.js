...
ScopedEvents.prototype.trigger = function(event) {
  ...
  // Need to clone the namespace array, otherwise
  // reverse screws up triggering parents
  var scopes = namespaces.map(function(scope) {
    return scope;
  }).reverse();
  ...
  for (var i = 0; i < namespaces_length; i++) {
    ...
    // clone scopes array
    var tempScope = scopes.map(function(scope) {
      return scope;
    });

    var triggeredChildScope = tempScope.splice(0, i).reverse().join(':');

    if (triggeredChildScope === '') {
      triggeredChildScope = null;
    }
    ...
    if (!!this._listeners[evt]) {
      ...
      for (var j = 0; j < listeners_length; j++) {
        ...
        listener.fn.apply(listener.ctx || this, args.concat(triggeredChildScope));
      }
    }
